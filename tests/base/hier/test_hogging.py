...
def test_hog_basic():
    ...
    assert Hog.Proem == "__"
    assert len(Hog.Proem) >= 2  # ensures genRid works

    rid = Hog._genRid()  # '__DqITqY1HgR8JA98qyvRW-S'
    assert rid[:2] == Hog.Proem[:2]
    assert len(rid) == 24
    ...
    assert hog.rid.startswith(Hog.Proem) # '__DqITqY1HgR8JA98qyvRW-S'
...
def test_hog_log(mockHelpingNowIso8601):
    """Test Hog class with logging rules"""
    Hog._clearall()  # clear Hog.Instances for debugging
    ...
    rid = '__DqITqY1HgR8JA98qyvRW-S'
    assert rid == '__DqITqY1HgR8JA98qyvRW-S'
    ...
    assert hog.header == ('rid\tbase\tname\tstamp\trule\tcount\n'
                    '__DqITqY1HgR8JA98qyvRW-S\tBoxerTest\tpig\t2021-06-27T21:26:21.233257+00:00\tevery\t0\n'
    ...
    [
        ...
        '__DqITqY1HgR8JA98qyvRW-S\tBoxerTest\tpig\t'
    ...
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
        ('__DqITqY1HgR8JA98qyvRW-S', 'BoxerTest','dog','2021-06-27T21:26:21.233257+00:00','once','0'),
        ('tyme.key', 'active.key', 'tock.key'),
        ('_boxer_BoxerTest_tyme', '_boxer_BoxerTest_active', '_boxer_BoxerTest_tock'),
        ('tyme.value', 'active.value', 'tock.value'),
        ('0.0', 'BoxTop', '0.03125')
    ...
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
        ('__DqITqY1HgR8JA98qyvRW-S','BoxerTest','cat','2021-06-27T21:26:21.233257+00:00','every','0'),
    ...
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
        ('__DqITqY1HgR8JA98qyvRW-S','BoxerTest','fox','2021-06-27T21:26:21.233257+00:00','update','0'),
        ('tyme.key', 'home.key', 'away.key'),
        ('_boxer_BoxerTest_tyme', 'location_home', 'location_away'),
        ('tyme.value', 'home.latN', 'home.lonE', 'away.latN', 'away.lonE'),
    ...
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
        ('__DqITqY1HgR8JA98qyvRW-S','BoxerTest','owl','2021-06-27T21:26:21.233257+00:00','change','0'),
        ('tyme.key', 'home.key', 'away.key'),
        ('_boxer_BoxerTest_tyme', 'location_home', 'location_away'),
        ('tyme.value', 'home.latN', 'home.lonE', 'away.latN', 'away.lonE'),
    ...
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
        ('__DqITqY1HgR8JA98qyvRW-S','BoxerTest','cow','2021-06-27T21:26:21.233257+00:00','span','0'),
        ('tyme.key', 'home.key', 'away.key'),
        ('_boxer_BoxerTest_tyme', 'location_home', 'location_away'),
        ('tyme.value', 'home.latN', 'home.lonE', 'away.latN', 'away.lonE'),
        ('0.0', '40.0', '-113.0', '42.0', '8.0')
...
def test_hog_cycle_size(mockHelpingNowIso8601):
    ...
    # vector locations as hits with default rule every
    hog = Hog(name=name, iops=iops, hold=hold, temp=True, rid=rid, flushForce=True,
              cycleCount=count, cycleSpan=period, cycleSize=size,
              home=homeKey, away=awayKey)
    assert hog.rule == Rules.every
    assert not hog.started
    assert not hog.onced
    assert hog.flushForce == True
    assert hog.cycleCount == count
    assert hog.cycleSpan == period == 0.0625
    assert hog.cycleSize == size
    assert hog.rid == rid

    assert hog.hits == {'home': 'location_home', 'away': 'location_away'}
    assert len(hog.cyclePaths) == count
    ...
    assert hog.first == 0.0
    assert hog.last == 0.0
    assert hog.flushLast == 0.0
    assert hog.cycleLast == None
    assert hog.rid == rid
    assert hog.stamp == dts

    assert os.path.getsize(hog.path) == 264
    ...
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
        ('__DqITqY1HgR8JA98qyvRW-S','BoxerTest','rat','2021-06-27T21:26:21.233257+00:00','every','2'),
        ('tyme.key', 'home.key', 'away.key'),
        ('_boxer_BoxerTest_tyme', 'location_home', 'location_away'),
        ('tyme.value', 'home.latN', 'home.lonE', 'away.latN', 'away.lonE'),
        ('0.0', '45.0', '-90.0', '40.0', '10.0')
    ]

    # run again
    tymist.tick()
    hold[tymeKey].value = tymist.tyme
    hold[awayKey].latN = 41.0
    hold[awayKey].lonE = 11.0
    ...
    assert os.path.getsize(hog.path) == 239  # only header
    assert os.path.getsize(hog.cyclePaths[0]) == 293  # over cycleSize
    ...
    cycle1 = open(hog.cyclePaths[0], "r")
    lines = cycle1.readlines()
    lines = [tuple(line.rstrip('\n').split('\t')) for line in lines]
    assert lines == \
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
    ...
    assert os.path.getsize(hog.path) == 267
    assert os.path.getsize(hog.cyclePaths[0]) == 293  # over cycleSize
    assert os.path.getsize(hog.cyclePaths[1]) == 0
    ...
    [
        ...
        ('_boxer_BoxerTest_tyme', 'location_home', 'location_away'),
        ('tyme.value', 'home.latN', 'home.lonE', 'away.latN', 'away.lonE'),
        ('0.0625', '45.0', '-90.0', '42.0', '12.0')
    ]

    # run again
    tymist.tick()
    hold[tymeKey].value = tymist.tyme
    hold[awayKey].latN = 43.0
    hold[awayKey].lonE = 13.0
    ...
    assert os.path.getsize(hog.path) == 239
    assert os.path.getsize(hog.cyclePaths[0]) == 296  # over cycleSize
    assert os.path.getsize(hog.cyclePaths[1]) == 293
    ...
    [
        ...
        ('tyme.value', 'home.latN', 'home.lonE', 'away.latN', 'away.lonE'),
        ('0.0625', '45.0', '-90.0', '42.0', '12.0'),
        ('0.09375', '45.0', '-90.0', '43.0', '13.0')
    ]


    cycle2 = open(hog.cyclePaths[1], "r")
    lines = cycle2.readlines()
    lines = [tuple(line.rstrip('\n').split('\t')) for line in lines]
    assert lines == \
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
...
def test_hog_cycle_span(mockHelpingNowIso8601):
    ...
    [
        ('rid', 'base', 'name', 'stamp', 'rule', 'count'),
        ('__DqITqY1HgR8JA98qyvRW-S','BoxerTest','bat','2021-06-27T21:26:21.233257+00:00','every','2'),
        ('tyme.key', 'home.key', 'away.key'),
        ('_boxer_BoxerTest_tyme', 'location_home', 'location_away'),
        ('tyme.value', 'home.latN', 'home.lonE', 'away.latN', 'away.lonE'),
        ('0.0', '45.0', '-90.0', '40.0', '10.0')
    ...
    assert os.path.getsize(hog.path) == 293
    ...
    assert os.path.getsize(hog.path) == 239  # just header
    assert os.path.getsize(hog.cyclePaths[0]) == 321
    assert os.path.getsize(hog.cyclePaths[1]) == 0
    ...
    assert os.path.getsize(hog.path) == 268
    assert os.path.getsize(hog.cyclePaths[0]) == 321
    assert os.path.getsize(hog.cyclePaths[1]) == 0
    ...
    tymist.tick()
    hold[tymeKey].value = tymist.tyme
    hold[awayKey].latN = 44.0
    hold[awayKey].lonE = 14.0
    ...
    assert os.path.getsize(hog.path) == 239
    assert os.path.getsize(hog.cyclePaths[0]) == 295
    assert os.path.getsize(hog.cyclePaths[1]) == 321
