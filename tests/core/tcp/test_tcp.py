...
def test_tcp_basic():
    ...
    """
    client = Client()
    assert isinstance(client.cycler, tasking.Cycler)
    assert client.timeout == 0.0
    assert isinstance(client.tymer, tasking.Tymer)
    ...
    server = Server()
    assert isinstance(server.cycler, tasking.Cycler)
...
def test_client_auto_reconnect():
    ...
    """
    cycler = tasking.Cycler(tick=0.05)
    with openServer(cycler=cycler, ha=("", 6101)) as server, \
